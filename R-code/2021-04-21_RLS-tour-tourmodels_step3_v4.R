...
load(file = paste0(data_folder, "/tourism_models6.rda"))
...
## Compare model SOS
tour_gam1 <- gam(bio_resids ~ s(NEOLI_Total,  k = 5) + 
                    s(TI, k = 5) +
                    has_fee +
...
# tour_gam1.1l <- gam(bio_resids ~ NEOLI_Total + 
#                       TI +
#                       has_fee + 
#                       s(Lat, Lon, bs = "sos", k = 100), data = MPAdat)

AIC(tour_gam1, tour_gam1.0, tour_gam1.1, tour_gam1.1k)#, #tour_gam1.1k, tour_gam1.1m, tour_gam1.1l)

# par(mfrow = c(2,2))
# gam.check(tour_gam1.0)
# gam.check(tour_gam1.1k)
# 
# par(mfrow = c(2,3))
# visreg(tour_gam1.0)
# visreg(tour_gam1.1k) # pretty sure this is over fitted

summary(tour_gam1)
summary(tour_gam1.0)
summary(tour_gam1.1)
summary(tour_gam1.1k)


## Stepwise selection to find the best model

tour_gam1.2 <- update(tour_gam1, . ~ . - s(TI, k = 5)) 
tour_gam1.3 <- update(tour_gam1, . ~ . - has_fee)
tour_gam1.4 <- update(tour_gam1, . ~ . - s(NEOLI_Total,  k = 5))
tour_gam1.5 <- update(tour_gam1, . ~ . - s(TI, k = 5) + TI - s(NEOLI_Total,  k = 5) + NEOLI_Total) 

# save(tour_gam1.0, tour_gam1.11, tour_gam1.2, tour_gam1.3, tour_gam1.4,
#      tour_gam1.5, tour_gam1, tour_gam1.1, tour_gam1.1k,
#      tour_select, tour_cs, file = paste0(data_folder, "/tourism_models6.rda"))

# Retain all effects
AIC(tour_gam1, tour_gam1.2, tour_gam1.3, tour_gam1.4, tour_gam1.5)


par(mfrow = c(2,2))
tour_mod <- tour_gam1.5
gam.check(tour_mod)
summary(tour_mod)
...
model_decription <- c("Tourism Index, Fee and NEOLI" = "tour_gam1.5",
                      "Fee and NEOLI" = "tour_gam1.2",
                      "Tourism Index and NEOLI" = "tour_gam1.3",
                      "Tourism Index and Fee" = "tour_gam1.4"
)
...
p1 <- ggplot(MPAdat) + aes(x = Lon, y = Lat) + theme_bw() +
  ...
  scale_colour_gradient(low = "purple",
                        ...
                        na.value = "red")
# p2 <- ggplot(MPAdat) + aes(x = Lon, y = Lat) + theme_bw() +
#   borders("world",fill="black",colour="black") +
#   geom_jitter(mapping = aes(col = sign(resid(tour_mod_comp)),
#                            #size = abs(resid(tour_mod_comp))
#                            ),
#              alpha = 0.5, width = 5,
#              height = 5) +
#   scale_colour_gradient(low = "purple",
...
# Load CB's semivariance function
source("~/Documents/Active_Projects/MaxRTools/semivariance_function.R")
